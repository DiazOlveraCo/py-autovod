...
class Processor:
    ...
    def _process_single_file(self, video_path):
        ...
        try:
            ...
            logger.info(f"Using LLM model: {model_name} (temp: {temperature}, max tokens: {max_tokens})")
            ...
            if not os.path.exists(transcription_json):
                logger.error(f"Error: Expected transcription file {transcription_json} was not generated")
                return
