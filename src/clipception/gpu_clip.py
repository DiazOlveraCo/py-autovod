...
def rank_clips_chunk(
    clips: List[Dict], api_key: str, site_url: str = "", site_name: str = ""
) -> str:
    client = OpenAI(
...
def rank_all_clips_parallel(
    ...
    chunk_data = [
        (chunk, api_key, site_url, site_name, i) for i, chunk in enumerate(chunks)
    ]
...
def transcribe_clips(
    ...
    try:
        ...
        ranked_clips = rank_all_clips_parallel(
            clips, api_key, site_url, site_name, chunk_size, num_processes
        )
...
def main():
    ...
    parser.add_argument(
        "--site_url", default="http://localhost", help="Site URL for OpenRouter API"
    )
    parser.add_argument(
        "--site_name", default="Local Test", help="Site name for OpenRouter API"
    )
    parser.add_argument(
